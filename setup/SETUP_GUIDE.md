...
| 4     | Install 255 manifest entries (254 active + 1 disabled) through the security-gated pipeline; combined with extension/CLI-provisioned skills this yields 281 dirs in `~/.agents/skills/` (plus 14 parked in `skills._disabled/`)                                                                          | ~5 min  |
...
**255 manifest entries (254 active + 1 disabled) + extension/CLI-provisioned and hand-ported skills = 281 dirs in `~/.agents/skills/`** (discovered by VS Code Copilot Chat), with **14 more parked** in `skills._disabled/` (1 upstream-disabled + 13 Copilot-incompatible, see v1.9.10 in the changelog):
