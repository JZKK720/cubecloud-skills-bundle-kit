...
| 4     | Install 192 manifest entries (191 active + 1 disabled) through the security-gated pipeline; combined with extension/CLI-provisioned skills this yields 236 dirs in `~/.agents/skills/`                                                                          | ~5 min  |
...
**191 manifest entries (191 active + 1 disabled) + 45 extension/CLI-provisioned and hand-ported skills = 236 dirs in `~/.agents/skills/`** (discovered by VS Code Copilot Chat):
