...
description: "Push an interface past conventional limits using the full power of the platform - a table handling a million rows, a dialog morphing from its trigger, streaming form validation, cinematic page transitions. Use when a surface should feel technically extraordinary. Requires judging what extraordinary means for the specific project, since the same effect that impresses on a portfolio embarrasses on a settings page."
license: Apache-2.0
---
...
2. **Get the user's pick before writing any code.** STOP and ask the user
