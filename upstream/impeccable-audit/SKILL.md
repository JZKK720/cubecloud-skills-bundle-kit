...
description: "Run systematic technical quality checks on a web interface and generate a scored report without fixing anything. Use when auditing implementation quality rather than critiquing design taste. Scores five dimensions 0-4 - accessibility, performance, responsive behavior, code quality, and theming - and documents findings for other procedures to address."
license: Apache-2.0
---
...
**Web only.** Native platforms (`ios` / `android` / `adaptive`) route to the native audit checklist instead; if the project is native, switch to it now.
