---
name: impeccable-adapt-native
description: "Recontextualize an existing native design (iOS / Android / adaptive) for a different device class, orientation, platform, or origin. Use when adapting a native app, where the trap is treating adaptation as scaling. Covers size classes, window size classes, posture and hinge handling on foldables, and translating idioms rather than transplanting them between iOS and Android."
license: Apache-2.0
---

Adapt an existing **native** design (`ios` / `android` / `adaptive`) to a different context: another device class, orientation, platform, or origin. The trap is treating adaptation as scaling. The job is rethinking the experience for the new context, inside the platform conventions of the iOS platform reference / the Android platform reference; read the target platform's reference before planning if Setup hasn't already.
