---
name: impeccable-audit-native
description: "Run systematic technical quality checks on a native app (iOS / Android / adaptive) and generate a scored report without fixing anything. Use when auditing native implementation quality from source (SwiftUI, UIKit, Compose, React Native, Flutter). Scores five dimensions 0-4 against platform conventions, including VoiceOver and TalkBack accessibility."
license: Apache-2.0
---
...
This is a code-level audit, not a design critique. Audit from source (SwiftUI / UIKit / Compose / React Native / Flutter); no browser tooling or `impeccable detect` applies. Score against the platform reference(s): the iOS platform reference / the Android platform reference, both for `adaptive`. Read them before scoring if Setup hasn't already. The report skeleton mirrors the web audit checklist; keep the two in sync when changing it.
