...
description: "Introduce color into an interface as hierarchy, meaning, and atmosphere rather than decoration. Use when a surface is under-colored, when color needs to encode action, selection, status, or wayfinding, or when a palette needs to carry atmosphere. Preserves confirmed brand and semantic conventions instead of replacing the visual world."
license: Apache-2.0
---
...
If a new identity is required, use the new-surface workflow. Ask only when a binding brand decision cannot be inferred.
...
Add at most two variant-specific parameters, such as palette, temperature, or tint behavior. Follow live browser iteration's parameter contract.
